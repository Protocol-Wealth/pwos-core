...
- **20 packages published** to npm under `@protocolwealthos/*` (Apache 2.0 / OIN / patent-pending); the 21st manifest, `onchain-accounting-contract`, is versioned at `0.2.0` and awaits maintainer-local npm publication and registry verification.
- **`onchain-accounting-contract@0.2.0` release prepared** — strict PII-free TypeScript/runtime ABI for deployed Nexus accounting contract `0.2.0`, including authoritative runtime schemas, structural schema hints, exact decimal/aggregate validation, tri-state correlation, exact discovery, tool declarations, and synthetic golden fixtures. The version PR consumed the first-release Changeset; package and wire versions remain independent. Publication stays in the standard Changesets/local-publish flow.
