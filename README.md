...
- **Stream Z PII-tagging program** — canonical typed `PII_TAGS` map at pw-api; structural prompt-construction middleware; independent PII egress canary, with each service (pw-api, pw-os-v2, pw-portal-v2) running its own copy so the layers cannot share a bug.
...
|---|---|---|
| **`audit-log`** | Rule 204-2 books-and-records | Hash-chained append-only log; anomaly detectors; `assertApprovedByDifferentParty`; Postgres trigger SQL |
...
| **`email-archive`** | Rule 204-2 books-and-records | SHA-256 chain-of-custody hashing; retention enforcement; query evaluator |
...
| **`auth`** | State privacy access discipline | HS256 JWT (refuses `alg:"none"`; timing-safe); numeric-rank role guard; workspace domain assert; per-agent token sign/verify/scope with revocation |
...
| **`gcp-helpers`** | Structured-logging Reg S-P §248.30(b) adjacent | `createCloudLogger()` (JSON-line structured logging); `pickConnectionStrategy()` (Cloud SQL IAM auth, refuses silent password fallback); `createCachingSecretLoader()`; `buildFrontendErrorReport()` |
...
The `@protocolwealthos/*` packages are what's published. You can use the packages standalone or compose them into your own platform.
...
| **`@protocolwealthos/email-archive`** | Archive primitives · chain-of-custody hashing · retention enforcement · query evaluator |
...
| **PII egress canary** | One copy per service: pw-api, pw-os-v2 and pw-portal-v2 each run their own copy of the egress canary; deliberately not a shared module so the layers cannot share a bug | [pwos.app/build](https://pwos.app/build) — AI-surface design record; paired with `@protocolwealthos/pii-guard` + `@protocolwealthos/ai-guardrails` |
