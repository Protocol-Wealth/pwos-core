...
2. **Consume `@protocolwealthos/disclosure-card` in the PWOS Compliance
   ...
   - **What to do:** import `parseDisclosureCard`,
     ...
     `@protocolwealthos/disclosure-card`. Persist the
...
1. **Publish PW's OWN disclosure card at `/disclosures`.**
   - Surface: `pwos.app/disclosures` (advisor- and partner-facing) + a
     ...
     `@protocolwealthos/disclosure-card` (post-publish, from the
     installed npm package), then overriding every field with PW's
     real values (firm = "Protocol Wealth LLC", crd = "335298",
     model = "claude-sonnet-4-6" or current pin, etc.). Pre-publish, the
     monorepo workspace dep `"@protocolwealthos/disclosure-card":
     "workspace:*"` resolves locally.
   ...
   - **Where:**
     - Card source-of-truth: `pw-os-v2/apps/api/src/disclosures/pw-card.ts`
       (or similar) authored by Nick (CISO) + CCO-reviewed. Imports
       `parseDisclosureCard` + `EXAMPLE_DISCLOSURE_CARD` from
     ...
     - Pre-publish CI gate: `assertNoVerifyMarkers(card)` at build time
       on both surfaces (also imported from
       `@protocolwealthos/disclosure-card`).
